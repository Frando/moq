...
and the V4L2 M2M codec of an ARM SoC (a Raspberry Pi's VideoCore, and the
equivalent block elsewhere) behind `v4l2`, since neither backend has been
validated on real hardware. To build `capture` without any of them (software
openh264 + V4L2 camera capture only), drop the default features. `capture`
itself still needs libclang and the V4L2 headers for the camera, and ALSA for
the microphone:
